    ...
    internal class DataTypeSerializer
    {
        ...
        public byte returnTypeAndRawByteArray(ref object _data, out byte[] out_bytes)
        {
            ...
            else if (_type == typeof(List<string>))
            {
                ...
                {
                    _Globals._service.InsertBytes(ref out_bytes, BitConverter.GetBytes(arr_lengths[i]), ipos); ipos += 4;
                    _Globals._service.InsertBytes(ref out_bytes, Encoding.ASCII.GetBytes(lst_string[i]), ipos); ipos += arr_lengths[i];// lst_string[i].Length;
                }
                //return
                _data_type = 18;
            }
